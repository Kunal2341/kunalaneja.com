#!/usr/bin/env python3
"""
High-quality script to process i2g2ro_assets videos and create a combined GIF
...
def main():
    ...
    palette = output_dir / "palette_hq.png"
    combined_gif = output_dir / "i2g2ro_combined_hq.gif"
    ...
    print("Processing i2g2ro_assets videos with HIGH QUALITY settings...")
    ...
    # 6. Generate high-quality palette for GIF
    cmd_palette = [
        'ffmpeg', '-y',
        '-i', str(combined_mp4),
    ...
    if not run_command(cmd_palette, "Generating high-quality palette for GIF"):
        sys.exit(1)
    
    # 7. Convert to high-quality GIF using palette
    cmd5 = [
        'ffmpeg', '-y',
        '-i', str(combined_mp4),
        '-i', str(palette),
    ...
    if not run_command(cmd5, "Converting to high-quality GIF with palette"):
        sys.exit(1)
    
    # Clean up temporary files
    print("\nCleaning up temporary files...")
    temp_files = [aa_processed, bbbb_processed, screencast_processed, combined_mp4, filelist_path, palette]
    ...
    print("- High-quality palette generation")
    print("- Advanced dithering for smoother colors")
