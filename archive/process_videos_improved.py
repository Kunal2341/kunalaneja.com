#!/usr/bin/env python3
"""
Improved script to process i2g2ro_assets videos and create a combined GIF
"""
...
def main():
    ...
    combined_gif = output_dir / "i2g2ro_combined_improved.gif"
    ...
    print("Processing i2g2ro_assets videos with improved GIF quality...")
    ...
    # 7. Generate palette for better GIF quality
    cmd_palette = [
        'ffmpeg', '-y',
        '-i', str(combined_mp4),
    ...
    if not run_command(cmd_palette, "Generating palette for GIF"):
        sys.exit(1)
    
    # 8. Convert to GIF with palette
    cmd6 = [
        'ffmpeg', '-y',
        '-i', str(combined_mp4),
        '-i', str(palette),
    ...
    if not run_command(cmd6, "Converting to GIF with palette"):
        sys.exit(1)
    
    # Clean up temporary files
    print("\nCleaning up temporary files...")
    temp_files = [aa_processed, bbbb_processed, isaac_processed, screencast_processed, combined_mp4, filelist_path, palette]
    for temp_file in temp_files:
        if temp_file.exists():
            temp_file.unlink()
            print(f"Removed: {temp_file.name}")
    
    print(f"\n✓ Successfully created improved combined GIF: {combined_gif}")
